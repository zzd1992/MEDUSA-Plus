...
def medusa_forward(
    ...
    medusa_logits, logits = initialize_medusa(input_ids, model, past_key_values)
    new_token = 0

    for idx in range(max_steps):
        candidates, tree_candidates, medusa_buffers = generator.generate_candidates(
            medusa_logits,
            logits,
            temperature,
            posterior_threshold,
...
if __name__ == "__main__":
    ...
    args.model_id = (
        ...
        + "-posterior_threshold-"
        + str(args.posterior_threshold)
        + "-posterior_alpha-"
        + str(args.posterior_alpha)
        + "-top_p-"
        + str(args.top_p)
        + "-sampling-"
        + args.sampling
